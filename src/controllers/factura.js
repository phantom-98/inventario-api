...
const forProvidersFacs = async () => {
  const facturas = await Factura.find({
    format: "Recibido",
    provider: { $ne: null },
    $or: [{ typeId: 33 }, { typeId: 34 }, { typeId: 61 }],
  })
    .sort({ createdAt: "desc" })
    .populate("provider");
...
const createDte = async (data, document = "boleta") => {
  ...
  try {
    ...
    throw new Error("error in bucket");
  }
};
  ...
  try {
    ...
    let facturaReq = {
      ...
      client: {
        ...
        name: rData["client"]["fistName"]
          ? rData["client"]["fistName"] + " " + rData["client"]["lastName"]
          : null,
...
const createforWeb2 = async (req, res) => {
  ...
  try {
    const emisor = await Emisor.findById(process.env.EMISOR_UID);
    let data = dteBoletaMapping(
      rData["items"],
      rData["client"]["rut"].split(".").join(""),
      true,
      emisor
    );
    let file = await createDte(data);
...
const getReceivedDteforApi = async (req, res) => {
  const facturas = await Factura.find({ format: "Recibido" }).sort({
    createdAt: "desc",
  });
  res.json(facturas);
};
...
const receivedDetails = async (req, res) => {
  ...
  if (!factura.url) {
    ...
    try {
      let response = await fetch(
        process.env.OPENFACTURA_URL +
          `/${factura.emisorData.RUTEmisor}/${factura.typeId}/${factura.folio}/pdf`,
        requestOptions
      );
      ...
      factura.save();
      res.json({
        pdfUrl: `https://s3.amazonaws.com/oxfar.cl/received_${name}`,
      });
    } catch (error) {
...
const changeStatus = async (req, res) => {
  try {
    ...
    const factura = await Factura.findOne({ _id: id });
    factura.status =
      !factura.status || factura.status == "Pagada" ? "No Pagada" : "Pagada";
    factura.save();
    res.json(factura);
  } catch (error) {
    res.status(500).json(error);
...
const getPerMonthandProvider = async (req, res) => {
  ...
  const facturas2 = await Factura.find({
    format: "Recibido",
    status: "No Pagada",
    provider: { $ne: null },
    $or: [{ typeId: 33 }, { typeId: 34 }, { typeId: 61 }],
